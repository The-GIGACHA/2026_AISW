...
class Parameter:
    ...
    SPECIAL_IDX_START_1 = 1125
    SPECIAL_IDX_END_1   = 1230

    
    # (곡선구간) 혀용경로 1개 시작/종료
    CUREVE_IDX_START_1 = 370
    CUREVE_IDX_END_1   = 580
    CUREVE_IDX_START_2 = 790
    # CUREVE_IDX_END_2   = 970
    CUREVE_IDX_END_2   = 920
    CUREVE_IDX_START_3 = 1405
    CUREVE_IDX_END_3   = 1500
    CUREVE_IDX_START_4 = 1530
    CUREVE_IDX_END_4   = 1657

    CUREVE_IDX_START_5 = 1048
    CUREVE_IDX_END_5   = 1070

    # 횡단보도 구간 
    CROSSLINE_IDX_START_0 = 470
    CROSSLINE_IDX_END_0 = 507
    CROSSLINE_IDX_START_1 = 545
    CROSSLINE_IDX_END_1 = 575
    CROSSLINE_IDX_START_2 = 820
    CROSSLINE_IDX_END_2 = 845
    CROSSLINE_IDX_START_3 = 885
    CROSSLINE_IDX_END_3 = 920
    CROSSLINE_IDX_START_4 = 1415
    CROSSLINE_IDX_END_4 = 1445
    CROSSLINE_IDX_START_5 = 1470
    CROSSLINE_IDX_END_5 = 1505
...
class LatticePlanner:
    def __init__(self, init_node = True):
        ...
        self.special_s_ranges = [
            (self.spline_ref.s[Parameter.SPECIAL_IDX_START_1], self.spline_ref.s[Parameter.SPECIAL_IDX_END_1])
        ]

        # 허용경로 1개 시작/종료 지점 s (곡선구간)
        self.curve_s_ranges = [
            (self.spline_ref.s[Parameter.CUREVE_IDX_START_1], self.spline_ref.s[Parameter.CUREVE_IDX_END_1]),
            (self.spline_ref.s[Parameter.CUREVE_IDX_START_2], self.spline_ref.s[Parameter.CUREVE_IDX_END_2]),
            (self.spline_ref.s[Parameter.CUREVE_IDX_START_3], self.spline_ref.s[Parameter.CUREVE_IDX_END_3]),
            (self.spline_ref.s[Parameter.CUREVE_IDX_START_4], self.spline_ref.s[Parameter.CUREVE_IDX_END_4]),
            (self.spline_ref.s[Parameter.CUREVE_IDX_START_5], self.spline_ref.s[Parameter.CUREVE_IDX_END_5])
        ]

        # 힝단보도 구간
        self.crossline_s_ranges = [
            (self.spline_ref.s[Parameter.CROSSLINE_IDX_START_0], self.spline_ref.s[Parameter.CROSSLINE_IDX_END_0]),
            (self.spline_ref.s[Parameter.CROSSLINE_IDX_START_1], self.spline_ref.s[Parameter.CROSSLINE_IDX_END_1]),
            (self.spline_ref.s[Parameter.CROSSLINE_IDX_START_2], self.spline_ref.s[Parameter.CROSSLINE_IDX_END_2]),
            (self.spline_ref.s[Parameter.CROSSLINE_IDX_START_3], self.spline_ref.s[Parameter.CROSSLINE_IDX_END_3]),
            (self.spline_ref.s[Parameter.CROSSLINE_IDX_START_4], self.spline_ref.s[Parameter.CROSSLINE_IDX_END_4]),
            (self.spline_ref.s[Parameter.CROSSLINE_IDX_START_5], self.spline_ref.s[Parameter.CROSSLINE_IDX_END_5])
        ]

        # 끼어들기 구간 목록 (우측 0, 좌측 1)
        self.danger_ranges = [
            (self.spline_ref.s[Parameter.DANGER_IDX_START_0], self.spline_ref.s[Parameter.DANGER_IDX_END_0], 1),
            (self.spline_ref.s[Parameter.DANGER_IDX_START_1], self.spline_ref.s[Parameter.DANGER_IDX_END_1], 0),
            (self.spline_ref.s[Parameter.DANGER_IDX_START_3], self.spline_ref.s[Parameter.DANGER_IDX_END_3], 1),
            (self.spline_ref.s[Parameter.DANGER_IDX_START_4], self.spline_ref.s[Parameter.DANGER_IDX_END_4], 0),
            (self.spline_ref.s[Parameter.DANGER_IDX_START_5], self.spline_ref.s[Parameter.DANGER_IDX_END_5], 1)
        ]
        ...
        self.iou_objects = []  # 최신 프레임에서 (ns, x, y) 튜플 목록 저장
    ...
    def camera_callback(self, msg: MarkerArray):
        """
        /iou_fusion_markers 로부터 (ns, x, y) 상대좌표를 수집.
        - ns: 클래스명 (ex. 'car', 'truck', 'person' ...)
        - x, y: base_link 기준 상대 좌표 (m)
        """
        objs = []

        for m in msg.markers:
            # 실제 물체 박스(CUBE)만 사용. 텍스트 마커(TEXT_VIEW_FACING)는 제외
            if m.type != Marker.CUBE:
                continue

            # (선택) frame_id 체크: base_link 기준만 신뢰
            # if m.header.frame_id != "base_link":
            #     continue

            ns = m.ns if m.ns else ""     # 클래스 이름
            x  = float(m.pose.position.x) # 상대 X (앞)
            y  = float(m.pose.position.y) # 상대 Y (좌+ / 우-)

            objs.append((ns, x, y))

            # (옵션) class_id가 필요하면 points[1].x에 저장되어 있음 → 이렇게 꺼낼 수 있음:
            # if len(m.points) >= 2:
            #     class_id = int(round(m.points[1].x))
            #     # 필요 시 objs 대신 다른 구조에 함께 저장

        # 최신 프레임 결과로 교체
        self.iou_objects = objs

        # (디버그) 1초에 한 번 프린트
        rospy.loginfo_throttle(1.0, f"[iou_fusion] {len(self.iou_objects)} objs: {self.iou_objects}")
    ...
    def nearest_forward_obstacle_vrel(self):
        ...
        # ================= [추가] 카메라 객체를 s,d로 변환해 '횡단보도 s-구간' & '도로 d-밴드'로 필터 후 STOP =================
        # 단일차선(곡선) + 횡단보도 구간에서만 동작
        in_curve_single_lane = any(s0 <= ego_s <= s1 for (s0, s1) in self.curve_s_ranges)
        in_crosswalk_zone    = any(s0 <= ego_s <= s1 for (s0, s1) in self.crossline_s_ranges)
        if in_curve_single_lane and in_crosswalk_zone:
            objs = getattr(self, "iou_objects", [])
            if objs:
                VEHICLE_CLASSES = {"car", "truck", "bus"}
                # s,d 판정 파라미터 (필요시 수치만 조정)
                S_MARGIN   = 1.5                    # 횡단보도 s-구간 앞뒤 여유
                D_BAND     = Parameter.road_width * 2.5   # d=0(차로 중심) 기준 좌우 허용 밴드
                AHEAD_MAX  = 30.0                   # 이내 전방만 고려 (안전)
                for ns, x_rel, y_rel in objs:
                    # base_link 상대좌표 -> 월드
                    Xw, Yw = self.local_to_world(self.gps_x, self.gps_y, self.ego_yaw, x_rel, y_rel)
                    # 월드 -> 경로 s,d
                    s_obj  = self.find_nearest_s(Xw, Yw, mode='obs', center_s=ego_s)
                    d_obj  = self.get_frenet_d(s_obj, Xw, Yw)

                    # 전방에서만 체크
                    if not (0.0 <= (s_obj - ego_s) <= AHEAD_MAX):
                        continue

                    # 이 객체가 '횡단보도 s-구간'에 들어왔는지 (인덱스 기반) + '도로 d-밴드' 안인지
                    in_cross_s = any((s0 - S_MARGIN) <= s_obj <= (s1 + S_MARGIN) for (s0, s1) in self.crossline_s_ranges)
                    in_road_d  = (abs(d_obj) <= D_BAND)
                    if in_cross_s and in_road_d:
                        cls = (ns or "").lower().strip()
                        if cls not in VEHICLE_CLASSES:
                            rospy.loginfo_throttle(1.0,
                                f"[crosswalk s,d] STOP by non-vehicle '{cls}' @ s={s_obj:.1f}, d={d_obj:.1f}")
                            return None, True
        # ============================================================================================================
