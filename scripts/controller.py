    ...
    def run(self, ego_x, ego_y, ego_yaw, ego_gear, ego_ind, ego_vel, curvedvelocity, path_updated_flag):
        ...
        if self.path is not None:
            ...
            try:
                ...
                steering_rad = steering_rad - cross_rad  # [최종] 조향부호 게이트웨이서 반전 확정 → cross는 이론값(-)
            except Exception:
                pass
...
class Morai_Control_Node:
    def __init__(self, init_node=True):
        ...
        # [2026_AISW] 구독자 등록은 __init__ 끝(_register_subscribers)으로 이동 — 콜백 레이스 방지
        ...
        # ServiceProxy를 한 번만 생성 — [2026_AISW] 대회 허용 네트워크엔 EventCmd 없음 → 2초 대기 후 없으면 비활성
        try:
            rospy.wait_for_service('/Service_MoraiEventCmd', timeout=2.0)
            self._event_cmd = rospy.ServiceProxy('/Service_MoraiEventCmd', MoraiEventCmdSrv)
        except rospy.ROSException:
            self._event_cmd = None
            rospy.logwarn('[controller] /Service_MoraiEventCmd 없음 — 기어/모드 전환 비활성(맵 gear 고정이면 무관)')
        ...
        # [2026_AISW] 모든 속성 초기화 완료 후 구독 시작 (proj_UTM 등 콜백 레이스 방지)
    ...
    def change_gear(self, gear_value):
        try:
            ...
            if self._event_cmd is None:
                rospy.logwarn_throttle(5.0,'[controller] EventCmd 미가용 — 기어 전환 생략')
            else:
    ...
    def change_ctrl_mode(self, mode_value):
        ...
        try:
            ...
            if self._event_cmd is None:
                rospy.logwarn_throttle(5.0,'[controller] EventCmd 미가용 — 전환 생략')
            else:
                self._event_cmd(evt)
    ...
    def main(self):
        ...
        try:
            while not rospy.is_shutdown():
                ...
                if self.stop_index_1633 > 0 and current_index is not None and self.stop_index_1633 - 1 <= current_index <= self.stop_index_1633 + 9:
                ...
                if self.stop_index_1633 > 0 and current_index is not None and current_index >= self.stop_index_1633 - 1 and not self.is_stop_completed_1633:
                ...
                # [2026_AISW] 규정 longlCmdType 1: accel/brake 0~1 클램프
                self.ctrl_cmd_msg.accel = min(max(accel_cmd, 0.0), 1.0)
                self.ctrl_cmd_msg.brake = min(max(brake_cmd, 0.0), 1.0)
