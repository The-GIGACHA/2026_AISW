...
class MasterController:
    ...
    def __init__(self):
        ...
        self.jamming_zone_start = 1651  # 제밍구역 시작 인덱스
        
        self.jamming_zone_end = 1825    # 제밍구역 종료 인덱스
    ...
    def load_ref_map(self):
        """마스터에서 독립적으로 ref_path 로드"""
        json_file = os.environ.get('HL_MAP_JSON') or os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'map', 'kcity_map.json')
    ...
    def get_traffic_light_strategy(self):
        ...
        current_index = self.master_ego_index_global
        traffic_zone_1 = 141 <= current_index <= 223
        traffic_zone_2 = 1100 <= current_index <= 1170
